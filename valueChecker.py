...
class ValueChecker:
    ...
    def checkModelValue(self, i, j):
        ...
        return self.checkValue(
            model.data(model.index(i, j, QModelIndex()), Qt.ItemDataRole.EditRole)
        )
    ...
    def isCurrentPartValid(self, highlightErrors=False):
        ...
        if self.isLastRowEmpty():
            ...
            partValid = partValid and self.isRowValid(
                model.rowCount() - 1, highlightErrors
            )
            return partValid
    ...
    def isRowValid(self, RowNum, highlightErrors=False):
        ...
        for i in range(model.columnCount()):
            ...
            if highlightErrors and theValue == CellValue.ValueNotFloat:
                self.tableViewWidget.item(RowNum, i).setForeground(
    ...
    def isLastRowEmpty(self):
        ...
        for j in range(model.columnCount()):
            lastRowEmpty = lastRowEmpty and (
                self.checkModelValue(i, j) == CellValue.ValueNone
            )
                    ...
                    for k in range(len(currentTuple)):
                        ...
                        if cellStatus == CellValue.ValueNone:
                            strResult.append(
                                self.translate_str("Part ")
                                + str(partNumber)
                                + self.translate_str(" row ")
                                + str(j + 1)
                                + self.translate_str(" column ")
                                + str(k + 1)
    ...
    def setOkButtonState(self):
        if (
            self.geometryType == _PointGeometry
            and (self.tableViewWidget.rowCount()) >= 1
        ):
            return True
        elif (
            self.geometryType == _LineGeometry
            and (self.tableViewWidget.rowCount() - 1) >= 2
        ):
            return True
        elif (
            self.geometryType == _PolygonGeometry
            and (self.tableViewWidget.rowCount() - 1) >= 3
        ):
            return True
        else:
            return False
