...
class ChooseFeatureGUI(QDialog, Ui_chooseFeatureDialog):
    ...
    def configureDialog(self, featureList: list, layer: QgsVectorLayer):
        ...
        for i in range(len(self.__localFeatureList)):
            textValue = (
                self.translate_str("Feature with Id = ")
                + str(self.__localFeatureList[i].id())
                + "; "
            )
            if len(displayFieldsList) > 0:
                ...
                for j in range(len(displayFieldsList)):
                    attributeValue = str(
                        self.__localFeatureList[i][displayFieldsList[j][0]]
                    )
    ...
    def selectDisplayFields(self):
        ...
        if localFields.count() == 0:
            ...
            for i in range(len(localFields)):
                priority = 0
                # If at least one field in selected features not empty
                # and can convert to string then continue
                if (
                    not self.fieldIsEmpty(i)
                    and self.fieldCanConvertToStr(i)
                    and (
                        localFields.field(i).type() == QVariant.String
                        or localFields.field(i).type() in numTypes
                        or localFields.field(i).type() in datetimeTypes
                    )
                ):
                    # If field name highly likely description of feature
    ...
    def fieldCanConvertToStr(self, fieldIndex):
        canConvert = True
        for i in range(len(self.__localFeatureList)):
            canConvert = canConvert and (lambda v: v is not None)(
                self.__localFeatureList[i][fieldIndex]
            )
    ...
    def fieldIsPreferred(self, fieldName):
        ...
        for i in range(len(preferredFieldNames)):
            isPreferred = isPreferred or (
                fieldName.lower().find(preferredFieldNames[i].lower()) != -1
            )
    ...
    def onFeatureChanged(self, currentRow):
        if currentRow != -1:
            if self.selectedFeature != -1:
                self.__layer.deselect(
                    self.__localFeatureList[self.selectedFeature].id()
                )
